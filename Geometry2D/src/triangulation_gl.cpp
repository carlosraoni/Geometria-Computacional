...
typedef enum{POLYGON_EDIT, TRIANGULATION} PROGRAM_MODE;
...
void drawTriangulation(PolygonTriangulation & t){
	int n = t.getNumberOfTriangles();
	for(int i = 0; i < n; i++)
		drawPolygon(t.getTriangle(i));
}

void drawPoints(const vector<Point2D> & p){
	int n = p.size();
	if(n == 0) return;
...
// Algoritmo
EarRemovalAlgorithm earRemoval;
...
void runTriangulation(){
	if(mode != TRIANGULATION) return;

	cout << "Running Triangulation" << endl;

	t = earRemoval.triangulate(polygon);

	cout << "Ear Removal Algorithm Executed" << endl;
	mode = TRIANGULATION;
}

void endPolygonEditMode(){
	if(polygon.size() < 3) return;

	cout << "Polygon Edit Finished" << endl;
	cout << "Starting Triangulation Mode" << endl;
	mode = TRIANGULATION;
}
...
int main(int argc, char **argv)
{
	...
	cout << "--- Polygon Triangulation How to Use ---" << endl << endl;
	cout << "Mouse Actions: " << endl << endl;
	cout << "\t Left Click (POLYGON_EDIT MODE) - Generate Polygon Point on Click Position" << endl;
	cout << "\t Left Click (TRIANGULATION MODE) - Runs Ear Removal Triangulation Algorithm" << endl;
	cout << "\t Right Click - Change Mode from POLYGON_EDIT to TRIANGULATION" << endl;
	...
	cout << "\t R - Runs Ear Removal Triangulation Algorithm" << endl;
	cout << "\t ESC - Exit Program" << endl << endl;
     ...
     Depth buffer */
  	...
  	/* Open a window */
  	window = glutCreateWindow("Polygon Triangulation Algorithm Tests");
